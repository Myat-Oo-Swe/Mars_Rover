...
-----
  ...
  python benchmark.py --csv ''           (disable CSV export)
  python benchmark.py --size 30
  python benchmark.py --min-dist 5 --max-dist 27   (override distance filter)
...
def print_head_to_head(
    ...
    def _signed(v: float) -> str:
        """Format a float with explicit sign, avoiding the '+-' artifact."""
    ...
    print(f"    Only {label1} succeeds : {_pct(only_m1, n)}")
    print(f"    Only {label2} succeeds : {_pct(only_m2, n)}")
    print(f"    Both fail            : {_pct(both_fail, n)}")

    # Efficiency gap on missions both succeeded
    gap1 = sum(
...
def export_csv_dual(
    ...
    print(f"\n  CSV saved → {path}  ({len(r1)} rows, both models)")
...
def main():
    ...
    parser.add_argument("--model2",      default=None,
                        help="Second model to compare (optional, no .zip)")
    ...
    parser.add_argument("--min-dist",   type=int, default=14,
                        help="Min Chebyshev distance between start and goal (default 14 — long routes)")
    ...
    parser.add_argument("--csv",        default="benchmark_results.csv",
                        help="CSV file to store per-mission results "
                             "(default: benchmark_results.csv, pass '' to disable)")
    ...
    if args.model2:
        ...
        # For the old model trained without VecNormalize, auto-detect but
        # also respect --normalizer2 'none' to force raw obs explicitly.
        if args.normalizer2 and args.normalizer2.lower() == "none":
            norm2 = None
        elif args.normalizer2:
            norm2 = args.normalizer2
        else:
            # Try to find a normalizer next to model2; old models won't have one
            norm2 = None
    ...
    while i < args.missions:
        ...
        # Random start/goal with Chebyshev distance constraint
        found = False
        for _ in range(500):
        ...
        # Skip fully blocked terrain (A* has no path)
        _, opt_e = astar(terrain, start, goal)
        if np.isinf(opt_e):
            skipped += 1
            continue

        # Run both models on the SAME terrain / start / goal
    ...
    if args.csv:
        if predictor2:
            export_csv_dual(results1, results2, args.csv, m1_label, m2_label)
        else:
            export_csv(results1, args.csv)
